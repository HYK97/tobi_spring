...
//		} finally {
//		}
		this.jdbcTemplate.update("insert into user values(?,?,?,?,?,?)", user.getId(), user.getName(), user.getPassword(),user.getLevel().intValue(),user.getLogin(),user.getRecommend());
	...
	public int update(User user) {
		...
		return jdbcTemplate.update("update user set name =?,password =? ,level =?,login =?,recommend=? where id=?", user.getName(), user.getPassword(),user.getLevel().intValue(),user.getLogin(),user.getRecommend(),user.getId());
